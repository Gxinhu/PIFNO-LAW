...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x, gridy=None):
        ...
        if gridy is None:
            ...
            x = self.ifft2d(
                gridy, factor1, factor2, self.modes1, self.modes2
            ) / (size1 * size2)
        return x
    ...
    def ifft2d(self, gridy, coeff1, coeff2, k1, k2):
        ...
        k_x1 = (
            torch.cat(
                (
                    torch.arange(start=0, end=k1, step=1),
                    torch.arange(start=-(k1), end=0, step=1),
                ),
                0,
            )
            .reshape(m1, 1)
        ...
        k_x2 = (
            torch.cat(
                (
                    torch.arange(start=0, end=k2, step=1),
                    torch.arange(start=-(k2 - 1), end=0, step=1),
                ),
                0,
            )
            .reshape(1, m2)
        ...
        k1_tensor = torch.outer(gridy[:, :, 0].view(-1), k_x1.view(-1)).reshape(
            batchsize, n, m1, m2
        )
        k2_tensor = torch.outer(gridy[:, :, 1].view(-1), k_x2.view(-1)).reshape(
            batchsize, n, m1, m2
        )
        k = k1_tensor + k2_tensor
...
class SpectralConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Return to physical space
        x = torch.fft.irfftn(
            out_ft, s=(x.size(2), x.size(3), x.size(4)), dim=[2, 3, 4]
        )
        return x
...
class FourierBlock(nn.Module):
    def __init__(
        ...
        if activation == "tanh":
            ...
            self.activation = None  # pyrefly: ignore[bad-assignment]
        else:
